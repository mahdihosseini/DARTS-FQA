...
class EONSS(nn.Module):
    def __init__(self, dataset):
        ...
        if self.dataset.lower() == "focuspath":
            ...
            self.gdn3 = Gdn(32)
            self.conv4 = nn.Conv2d(32, 64, 1, stride=1, padding=0)
            self.gdn4 = Gdn(64)
            self.st2_fc1 = nn.Conv2d(64, 256, 1, stride=1, padding=0)
            self.st2_gdn1 = Gdn(256)
            self.st2_fc2 = nn.Conv2d(256, 1, 1, stride=1, padding=0)
        elif self.dataset.lower() == "deepfocus" or self.dataset.lower() == "bioimage64" or self.dataset.lower() == "focuspath64" or self.dataset.lower() == "tcga":
            self.conv1 = nn.Conv2d(3, 8, 5, stride=2, padding=2)
            self.gdn1 = Gdn(8)
            self.conv2 = nn.Conv2d(8, 16, 5, stride=2, padding=2)
            self.gdn2 = Gdn(16)
    ...
    def forward(self, x):
        batch_size = x.size()[0]
        if self.dataset.lower() == "focuspath":
            x = F.max_pool2d(self.gdn1(self.conv1(x)), (4, 4))
            x = F.max_pool2d(self.gdn2(self.conv2(x)), (2, 2))
            x = F.max_pool2d(self.gdn3(self.conv3(x)), (3, 3))
            x = F.max_pool2d(self.gdn4(self.conv4(x)), (1, 1))
